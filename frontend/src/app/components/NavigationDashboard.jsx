      ...
      <div data-guide="report-photo" className="mt-5 overflow-hidden rounded-[15px] border-2 border-dashed border-[#cfdcde] bg-[#fbfdfd] p-3 text-center text-[10px] text-[#71818c]">
        {photoPreview ? (
          <div className="w-full">
            <Image unoptimized width={280} height={150} src={photoPreview} alt="Preview foto laporan" className="mx-auto h-32 w-full rounded-xl object-cover"/>
            <b className="mt-2 block truncate text-[#0c6478]">{photo?.name}</b>
            <div className="mt-2 grid grid-cols-2 gap-2">
              <label className="cursor-pointer rounded-[10px] bg-[#e9f7f4] px-2 py-2 font-bold text-[#0c6478]"><input type="file" accept="image/*" capture="environment" onChange={choosePhoto} className="sr-only" /><Camera className="mr-1 inline size-3" />Foto ulang</label>
              <label className="cursor-pointer rounded-[10px] bg-[#f0f3f5] px-2 py-2 font-bold text-[#52616b]"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={choosePhoto} className="sr-only" />Pilih lainnya</label>
            </div>
          </div>
        ) : (
          <div>
            <Camera className="mx-auto mb-2 size-5 text-[#0c6478]" />
            <b className="block text-[#344b55]">Tambahkan foto kondisi (wajib)</b>
            <small className="mt-1 block">Kamera tersedia di perangkat mobile · maksimal 5 MB</small>
            <div className="mt-3 grid grid-cols-2 gap-2">
              <label className="cursor-pointer rounded-[11px] bg-[#0c6478] px-2 py-2.5 font-extrabold text-white"><input type="file" accept="image/*" capture="environment" onChange={choosePhoto} className="sr-only" /><Camera className="mr-1 inline size-3" />Ambil foto</label>
              <label className="cursor-pointer rounded-[11px] border border-[#d7e1e3] bg-white px-2 py-2.5 font-extrabold text-[#52616b]"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={choosePhoto} className="sr-only" />Pilih galeri</label>
            </div>
          </div>
        )}
      </div>
...
export default function NavigationDashboard({ initialProfile="walking", initialDestination=null }) {
  ...
  return <main className={`relative h-dvh min-h-0 overflow-hidden bg-[#dfe5e8] sm:min-h-[620px] ${profile==='low-vision'?'contrast-[1.08]':''}`}>
    ...
    {panel==='directory'&&<DirectoryPanel selectedId={directoryDetail?.externalId} session={session} onLogin={()=>navigate('/masuk')} onClose={()=>{setDirectoryDetail(null);setPanel(null)}} onSelect={(place)=>{setDirectoryDetail(place);setDestinationCoordinates(place.coordinates)}} />}
    ...
    <HelpGuide onPanelChange={setPanel} />
  </main>;
}
